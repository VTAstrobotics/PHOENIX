...
If you are struggling to see a GUI in the developer container, ensure that you have run the command `xhost +local:root` *on your host*. I would recommend adding this to your `.bashrc` file so that it always runs (`echo "xhost + local:root" >> ~/.bashrc`).
