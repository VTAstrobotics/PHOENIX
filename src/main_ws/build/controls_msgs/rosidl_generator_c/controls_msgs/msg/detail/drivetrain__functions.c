// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from controls_msgs:msg/Drivetrain.idl
// generated code does not contain a copyright notice
#include "controls_msgs/msg/detail/drivetrain__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
controls_msgs__msg__Drivetrain__init(controls_msgs__msg__Drivetrain * msg)
{
  if (!msg) {
    return false;
  }
  // motors
  return true;
}

void
controls_msgs__msg__Drivetrain__fini(controls_msgs__msg__Drivetrain * msg)
{
  if (!msg) {
    return;
  }
  // motors
}

bool
controls_msgs__msg__Drivetrain__are_equal(const controls_msgs__msg__Drivetrain * lhs, const controls_msgs__msg__Drivetrain * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // motors
  for (size_t i = 0; i < 2; ++i) {
    if (lhs->motors[i] != rhs->motors[i]) {
      return false;
    }
  }
  return true;
}

bool
controls_msgs__msg__Drivetrain__copy(
  const controls_msgs__msg__Drivetrain * input,
  controls_msgs__msg__Drivetrain * output)
{
  if (!input || !output) {
    return false;
  }
  // motors
  for (size_t i = 0; i < 2; ++i) {
    output->motors[i] = input->motors[i];
  }
  return true;
}

controls_msgs__msg__Drivetrain *
controls_msgs__msg__Drivetrain__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  controls_msgs__msg__Drivetrain * msg = (controls_msgs__msg__Drivetrain *)allocator.allocate(sizeof(controls_msgs__msg__Drivetrain), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(controls_msgs__msg__Drivetrain));
  bool success = controls_msgs__msg__Drivetrain__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
controls_msgs__msg__Drivetrain__destroy(controls_msgs__msg__Drivetrain * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    controls_msgs__msg__Drivetrain__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
controls_msgs__msg__Drivetrain__Sequence__init(controls_msgs__msg__Drivetrain__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  controls_msgs__msg__Drivetrain * data = NULL;

  if (size) {
    data = (controls_msgs__msg__Drivetrain *)allocator.zero_allocate(size, sizeof(controls_msgs__msg__Drivetrain), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = controls_msgs__msg__Drivetrain__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        controls_msgs__msg__Drivetrain__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
controls_msgs__msg__Drivetrain__Sequence__fini(controls_msgs__msg__Drivetrain__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      controls_msgs__msg__Drivetrain__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

controls_msgs__msg__Drivetrain__Sequence *
controls_msgs__msg__Drivetrain__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  controls_msgs__msg__Drivetrain__Sequence * array = (controls_msgs__msg__Drivetrain__Sequence *)allocator.allocate(sizeof(controls_msgs__msg__Drivetrain__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = controls_msgs__msg__Drivetrain__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
controls_msgs__msg__Drivetrain__Sequence__destroy(controls_msgs__msg__Drivetrain__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    controls_msgs__msg__Drivetrain__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
controls_msgs__msg__Drivetrain__Sequence__are_equal(const controls_msgs__msg__Drivetrain__Sequence * lhs, const controls_msgs__msg__Drivetrain__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!controls_msgs__msg__Drivetrain__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
controls_msgs__msg__Drivetrain__Sequence__copy(
  const controls_msgs__msg__Drivetrain__Sequence * input,
  controls_msgs__msg__Drivetrain__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(controls_msgs__msg__Drivetrain);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    controls_msgs__msg__Drivetrain * data =
      (controls_msgs__msg__Drivetrain *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!controls_msgs__msg__Drivetrain__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          controls_msgs__msg__Drivetrain__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!controls_msgs__msg__Drivetrain__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
